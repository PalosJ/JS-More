...
public final class DinosaurAgeSystem {
    ...
    public static void registerAdultAge(ResourceLocation speciesId, double adultAgeYears) {
        if (speciesId == null || adultAgeYears <= 0.0D) {
            return;
        }
        SpeciesAgeProfile existing = OVERRIDE_AGE_PROFILES.get(speciesId);
        double adultGameDays = existing != null
                ? existing.adultGameDays()
                : resolveFallbackGameDays(null);
        OVERRIDE_AGE_PROFILES.put(speciesId, new SpeciesAgeProfile(adultAgeYears, adultGameDays));
    }
    ...
    public static void initializeSpawnEggAge(JSAnimalBase animal, boolean startAsBaby) {
        ...
        DinosaurAgeData data = animal.getData(JSReviseAttachments.DINOSAUR_AGE);
        ...
        animal.syncData(JSReviseAttachments.DINOSAUR_AGE);
    }
    ...
    public static DinosaurAgeEstimate estimate(JSAnimalBase animal) {
        ...
        if (!shouldForceAdultAgeFloor(animal, sizeProfile) && normalizedGrowth < 1.0D) {
            ...
            DinosaurAgeData data = animal.getExistingDataOrNull(JSReviseAttachments.DINOSAUR_AGE);
    ...
    private static SpeciesAgeProfile resolveSpeciesAgeProfile(
        ...
        SpeciesAgeProfile known = SPECIES_AGE_PROFILES.get(sizeProfile.speciesId().getPath());
