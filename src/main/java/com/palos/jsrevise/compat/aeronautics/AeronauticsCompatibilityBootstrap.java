...
public final class AeronauticsCompatibilityBootstrap {
    static final String LINKED_ADAPTER =
            "com.palos.jsrevise.compat.aeronautics.linked.ExactAeronauticsAdapter";
    ...
    private static void log(Result result) {
        ...
        switch (result.status()) {
            case READY -> JSRevise.LOGGER.info("JS-revise Aeronautics compatibility enabled: {}", diagnostic);
            case ABSENT -> JSRevise.LOGGER.debug("JS-revise Aeronautics compatibility inactive: {}", diagnostic);
            case DRIFT -> JSRevise.LOGGER.warn(
