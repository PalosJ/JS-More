...
public final class DinosaurCaptureCageItem extends BlockItem {
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (CaptureBoxAuthority.hasRecoveryMarker(stack) && !recoveryCarrier) {
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltip, TooltipFlag flag) {
        if (CaptureBoxAuthority.hasRecoveryMarker(stack)) {
            tooltip.add(Component.translatable("tooltip.jsmore.dinosaur_capture_box.unreadable")
        ...
        if (inspection.state() == DinosaurCaptureItemData.InspectionState.UNREADABLE) {
            ...
            tooltip.add(Component.translatable(
                            "tooltip.jsmore.dinosaur_capture_box.occupied",
                            inspection.data().displayName()
                    )
                    .withStyle(ChatFormatting.GRAY));
        } else {
            tooltip.add(Component.translatable("tooltip.jsmore.dinosaur_capture_box.empty")
                    .withStyle(ChatFormatting.GRAY));
        }
    }
