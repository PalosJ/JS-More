...
public final class DinosaurCapturePlacementGameTests {
    ...
    public static void canonicalResolutionProtectsMalformedStructuresFromCascadeDeletion(GameTestHelper helper) {
        ...
        long survivors = placements.stream()
                .filter(placement -> helper.getLevel().getBlockState(placement.pos())
                        .is(JSMoreBlocks.DINOSAUR_CAPTURE_CAGE.get()))
                .count();
    ...
    public static void everyPartAndSideExposeOneStableValidatedHopperHandler(GameTestHelper helper) {
        ...
        ItemStack syringes = new ItemStack(JSMoreItems.ANESTHETIC_SYRINGE.get(), 2);
        ...
        ItemStack protectedRecovery = new ItemStack(JSMoreItems.DINOSAUR_CAPTURE_CAGE.get());
        net.minecraft.world.item.component.CustomData.update(
                net.minecraft.core.component.DataComponents.CUSTOM_DATA,
                protectedRecovery,
                tag -> tag.put("JSMoreRelocationRecovery", new net.minecraft.nbt.CompoundTag())
        );
        ...
        if (expected.insertItem(0, new ItemStack(JSMoreItems.ANESTHETIC_SYRINGE.get()), false).isEmpty()) {
    ...
    private static boolean previousStatesRestored(
        ...
        for (int index = 0; index < positions.size(); index++) {
            ...
            if (!current.equals(previousStates.get(index))
                    || current.is(JSMoreBlocks.DINOSAUR_CAPTURE_CAGE.get())
                    || current.is(JSMoreBlocks.BROKEN_DINOSAUR_CAPTURE_BOX.get())) {
                return false;
            }
        }
        return true;
