...
public final class DinosaurProfileResolver {
    private static final ResourceLocation UNKNOWN_SPECIES = JSMore.id("unknown");
    ...
    public static DinosaurSizeProfile resolve(JSAnimalBase animal) {
        ...
        if (override != null) {
            try {
                ...
                if (LOGGED_OVERRIDE_FAILURES.add(speciesId)) {
                    JSMore.LOGGER.warn("Ignoring invalid dinosaur profile override for {}", speciesId);
                }
            } catch (RuntimeException exception) {
                if (LOGGED_OVERRIDE_FAILURES.add(speciesId)) {
                    JSMore.LOGGER.warn("Ignoring failed dinosaur profile override for {}", speciesId, exception);
                }
            }
        }
        if (shouldLogInferredFallback(
            ...
            JSMore.LOGGER.info("Using inferred dinosaur profile for {}", speciesId);
        }
        return profile;
    }
    ...
    public static void auditRegisteredAnimals(ServerLevel level) {
        ...
        for (JSAnimal<?> registeredAnimal : JSAnimals.getAnimals()) {
            ...
            try {
                ...
                JSMore.LOGGER.warn("Unable to validate Jurassic Saga animal profile", exception);
            }
        }
        if (debugLogging()) {
            JSMore.LOGGER.info(
