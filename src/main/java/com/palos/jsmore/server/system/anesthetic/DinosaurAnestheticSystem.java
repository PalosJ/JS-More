...
import com.palos.jsmore.mixin.TravelersAnimalAnimationModuleAccessor;
import com.palos.jsmore.network.SleepAnimationGuardPayload;
...
public final class DinosaurAnestheticSystem {
    private static final String ANESTHETIC_SLEEP_SAVE_MARKER = "jsmore.anesthetic_sleeping";
    private static final String RAW_SLEEP_SAVE_MARKER = "jsmore.raw_sleeping";
    ...
    public static CompoundTag saveRelativeAnestheticState(JSAnimalBase animal) {
        ...
        AnestheticData data = animal.getExistingDataOrNull(JSMoreAttachments.ANESTHETIC);
    ...
    public static void restoreRelativeAnestheticState(
        ...
        if (relativeTag == null || relativeTag.isEmpty()) {
            animal.removeData(JSMoreAttachments.ANESTHETIC);
            animal.removeData(JSMoreAttachments.ANESTHETIC_FLOAT);
            return;
        }
        ...
        AnestheticData data = animal.getData(JSMoreAttachments.ANESTHETIC);
        data.deserializeRelativeNBT(null, relativeTag, baseGameTime, currentGameTime);
        if (data.isEmpty()) {
            animal.removeData(JSMoreAttachments.ANESTHETIC);
            animal.removeData(JSMoreAttachments.ANESTHETIC_FLOAT);
        } else {
            animal.syncData(JSMoreAttachments.ANESTHETIC);
        }
    }

    public static boolean isFloating(JSAnimalBase animal) {
        AnestheticFloatData data = animal.getExistingDataOrNull(JSMoreAttachments.ANESTHETIC_FLOAT);
    ...
    public static AnestheticVisualState resolveVisualState(JSAnimalBase animal) {
        AnestheticFloatData data = animal == null
                ? null
                : animal.getExistingDataOrNull(JSMoreAttachments.ANESTHETIC_FLOAT);
    ...
    public static void prepareAnimationSleepState(JSAnimalBase animal) {
        ...
        if (!animal.level().isClientSide && animal.hasData(JSMoreAttachments.ANESTHETIC)) {
    ...
    public static void syncSleepStateForTracking(JSAnimalBase animal) {
        ...
        if (bridgeSleepState) {
            animal.syncData(JSMoreAttachments.ANESTHETIC);
            animal.syncData(JSMoreAttachments.ANESTHETIC_FLOAT);
        }
    ...
    public static void tickServer(JSAnimalBase animal) {
        ...
        AnestheticFloatData existingFloatData = animal.getExistingDataOrNull(JSMoreAttachments.ANESTHETIC_FLOAT);
        ...
        if (!AnestheticStateService.update(animal)) {
            ...
            AnestheticData anestheticData = animal.getExistingDataOrNull(JSMoreAttachments.ANESTHETIC);
        ...
        AnestheticFloatData floatData = existingFloatData != null
                ? existingFloatData
                : animal.getData(JSMoreAttachments.ANESTHETIC_FLOAT);
    ...
    private static void stabilizeCommonState(JSAnimalBase animal) {
        AnestheticFloatData floatData = animal.getData(JSMoreAttachments.ANESTHETIC_FLOAT);
    ...
    private static int clearNonSleepAnimationState(TravelersAnimalAnimationModule animationModule) {
        ...
        return clearNonSleepAnimationTransitions(accessor.jsmore$getAnimationMap());
    }
    ...
    private static boolean debugLogging() {
        try {
            return JSMoreConfig.DEBUG_LOGGING.get();
