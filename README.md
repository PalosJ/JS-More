...
- 箱体耐久最大 20。处于麻醉保护区间时不消耗；其余时间每 20 tick 扣 1。手持、背包、打开容器、掉落物和已放置方块均参与结算，未加载容器和区块不会被全局扫描。显式 500 容量数据按剩余比例向上取整迁移到 20，例如 `375/500 → 15/20`、`1/500 → 1/20`；旧 100 容量数据先按 1.0.124 的既定规则保留已损耗点数映射到 500（`75 → 475`、`1 → 401`），再分别迁移为 `19/20`、`17/20`，直接升级与逐版本升级结果一致。任一旧容量的 0 都保持 0；首次服务端扫描会真正写回 `DurabilityCapacity=20`。HUD 和现有有效箱物品 tooltip 只把精确剩余值格式化为整数百分比；标准 `DAMAGE`/`MAX_DAMAGE`、Damage mirror、原版耐久条和第三方耐久模组的精确数据入口继续同步到 20。
...
权威耐久仍在捕获载荷中；当前 `DurabilityCapacity=20`、`MAX_DAMAGE=20` 与 `DAMAGE=20-耐久` 共同维护原版耐久条和 Inventory HUD+ 的兼容镜像，旧 100/500 容量会按上述规则迁移并写回。正数被动流逝继续使用包含饥饿与麻醉投影的纯数据 fast path，不每秒物化实体或改写 `CUSTOM_DATA`/Damage mirror；旧数据懒迁移、阈值到达、pending 到期、精确结算和耐久归零才进入必要的持久化或实体路径。
