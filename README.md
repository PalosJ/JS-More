...
本文档以 `1.0.109` 源码为准，面向后续维护、问题排查和功能扩展。
...
```text
build/libs/jsrevise-1.0.109.jar
```
...
- 破损的恐龙捕获箱仍是纯视觉测试方块，也作为捕获箱耐久归零释放后的残骸。它现在按正式捕获箱的朝向和 footprint 放置为 16 个无碰撞 part，主箱体整体可选中和破坏，破坏任意 part 会清理整组；物品模型主箱体与正式捕获箱保持相同范围和 GUI 缩放，周围散落碎片按放置态 BER 的前门、两侧和顶部碎片位置关系映射到物品模型，碎片随箱体同比例显示并允许超出原版 `0..16` 物品栏框；放置态通过视觉方块实体渲染 2×4×2 破损外壳和单面 flat debris 碎片，碎片分布在 front、back、left side、right side 与 top 破洞外侧，不再穿入主箱体。破损贴图已补齐 front、back、side 和 top 相邻边缘的角破损连续性，靠近后门的破损角也会影响后门面；两个长侧面保留同一集装箱风格但使用不同破损轮廓。debris sheet 的碎片纹理按各自来源面采样，便于和散落位置的破损面对应。它不会捕获、释放或保存生物，也不会显示捕获箱 HUD、耐久或麻醉交互；生存模式可用石镐及以上采掘，挖掘速度按玻璃级别处理。
