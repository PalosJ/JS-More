...
本文档以 `1.0.112` 源码为准，面向后续维护、问题排查和功能扩展。
...
合成配方为上排 3 个铁锭，中排为线、绊线钩、线，下排中间 1 个铁锭。
...
麻醉弩保留原版弩从上方观察的正交俯视物品视角、待机状态和三段拉弦状态，但将材质替换为更新后的蓝灰工业机械风格像素贴图。10 张状态贴图均保持 16×16 物品资源尺寸，包含待机、三段拉弦以及 1 至 6 支剩余弹量状态；六档装填贴图逐档不同并与模型谓词顺序一一对应。弓臂、导轨和弦线继续使用清晰的单像素边缘与分层金属明暗，小型麻醉针保留前向针头和蓝色药液识别色。
...
合成配方为上排左右各 1 个铁粒，中排为玻璃、Jurassic Saga 指南书、玻璃。
...
合成配方为 7 个铁块包围中心小麦，并在中排右侧放置 1 个铁门。
...
```text
build/libs/jsrevise-1.0.112.jar
```
...
- 新增恐龙博士眼镜、恐龙捕获箱和麻醉弩的 3x3 工作台合成配方；配方资源使用 Minecraft 1.21.1 的 `data/jsrevise/recipe/` 单数目录和 `result.id` 字段。
- 项目已完成模块化拆分。
...
- 麻醉弩采用更新后的 16×16 蓝灰工业机械风格状态贴图，并保留原版正交俯视视角和完整拉弦动画结构；左键发射不再触发普通攻击挥手，六档剩余弹量模型与 `crossbow_loaded_1` 到 `crossbow_loaded_6` 一一对应且逐档不同。
