...
本文档以 `1.0.108` 源码为准，面向后续维护、问题排查和功能扩展。
...
`jsrevise:broken_dinosaur_capture_box` 是“破损的恐龙捕获箱”外观测试方块，也会作为捕获箱耐久归零并成功释放恐龙后的残骸。它只用于在游戏内查看破损箱建模和贴图：物品栏使用与正式捕获箱一致尺寸的复用方块贴图 3D baked 模型，并额外显示箱体周围的散落碎片；放置后以 16 个无碰撞 part 组成 2×4×2 主箱体，整体可被选中和破坏，但实体可以穿过。视觉方块实体渲染内缩后的破损外壳，以及分布在 front、back、side 和 top 破洞附近的单面 flat debris 碎片。当前物品栏模型内收了碎片坐标并降低 GUI 缩放，避免内容溢出物品栏框。当前破损资源补齐了相邻面的角破损连续性，尤其是靠近后门的破损角会同时影响相连侧面和后门面；两个长侧面的破损轮廓不再复用同一形状。碎片位置在主箱体外侧并使用对应破损面的纹理区域。碎片只做视觉效果，不参与碰撞、选中或掉落。该方块不接入捕获、释放、耐久、麻醉、HUD、网络或实体保存逻辑；生存模式可用石镐及以上采掘，挖掘速度按玻璃级别处理。
...
```text
build/libs/jsrevise-1.0.108.jar
```
...
- 破损的恐龙捕获箱仍是纯视觉测试方块，也作为捕获箱耐久归零释放后的残骸。它现在按正式捕获箱的朝向和 footprint 放置为 16 个无碰撞 part，主箱体整体可选中和破坏，破坏任意 part 会清理整组；物品模型范围与正式捕获箱一致，并在物品栏模型中显示箱体周围散落碎片，碎片坐标已内收且 GUI 缩放降低以避免溢出物品栏框；放置态通过视觉方块实体渲染 2×4×2 破损外壳和单面 flat debris 碎片，碎片分布在 front、back、left side、right side 与 top 破洞外侧，不再穿入主箱体。破损贴图已补齐 front、back、side 和 top 相邻边缘的角破损连续性，靠近后门的破损角也会影响后门面；两个长侧面保留同一集装箱风格但使用不同破损轮廓。debris sheet 的碎片纹理按各自来源面采样，便于和散落位置的破损面对应。它不会捕获、释放或保存生物，也不会显示捕获箱 HUD、耐久或麻醉交互；生存模式可用石镐及以上采掘，挖掘速度按玻璃级别处理。
