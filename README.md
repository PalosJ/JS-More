...
│  │  │  ├─ server/block/           恐龙捕获笼多方块和方块实体
│  │  │  ├─ server/entity/          麻醉针投射物
│  │  │  ├─ server/item/            麻醉针、麻醉弩、恐龙博士眼镜、捕获笼
│  │  │  ├─ server/registry/        方块、方块实体、物品、实体、附件、创造模式页注册
│  │  │  ├─ server/system/          年龄、麻醉、体型、捕获笼和世界生成系统
...
- `JSReviseBlocks` / `JSReviseBlockEntityTypes`：注册恐龙捕获笼多方块 controller 和方块实体。
- `JSReviseItems`：注册四个物品。
...
注册名：`jsrevise:dinosaur_capture_cage`

恐龙捕获笼用于临时收容处于 active 麻醉状态的 Jurassic Saga 生物。捕获条件严格使用
...
破坏任意 part 会清除整个结构，并只掉落一个捕获笼物品；创造模式破坏有恐龙的笼子也会像潜影盒一样掉落带捕获数据的物品，空笼在创造模式下不掉落。爆炸、命令替换或其他模组移除捕获笼时，会在方块实体仍可读取时兜底掉落带捕获数据的笼子；loot table 仍保持空表，避免和玩家手动数据掉落重复。方块属性按铁块复制，保留金属声音和无遮挡渲染，并通过方块标签要求铁镐或以上采掘。

捕获笼物品最大堆叠 1。空笼 tooltip 只显示“空”且隐藏耐久条；有恐龙时显示恐龙名称、笼体耐久、麻醉剩余时间和被捕获时长。笼体耐久不是原版 `Damage` 组件，也不会通过每秒改写 ItemStack 来驱动显示；客户端 tooltip 会按当前世界时间派生麻醉剩余、被捕获时长和笼体耐久，物品栏只使用自定义耐久条显示当前保存的基准耐久。背包或容器中的捕获笼不再为了时间流逝、临时实体 NBT、生命体征快照或正数耐久自然流逝每秒写回完整 `CUSTOM_DATA`；只有注射、喂食、补水、耐久首次归零、数据修复、放置或回收等真实持久状态变化才写回物品数据。手持有恐龙的捕获笼按住 Shift 右键方块，会在目标附近搜索安全位置并释放笼内恐龙；已放置的捕获笼不再支持空手 Shift 右键释放。直接放置有恐龙的捕获笼时，捕获数据会转移到方块实体。

笼子耐久最大 `1000`。笼内恐龙仍处于 active 麻醉时不消耗耐久；麻醉结束后每秒扣 1 点。放置方块、玩家背包和掉落物每秒结算，打开的容器走轻量懒结算，不扫描未加载容器或全世界区块。耐久归零时会在持有者、掉落物、容器或方块附近搜索安全位置自动释放；背包和容器中的笼子释放成功后物品消失，掉落物释放成功后丢弃掉落实体，已放置笼子释放成功后无掉落拆除整个 2×4×2 笼体。自动释放成功会先清除物品栈中的捕获数据，并通过 `StackSettlementResult.consumesCarrier()` 要求调用方消费载体，避免释放成功后客户端或容器短暂保留 0 耐久捕获笼。耐久首次归零但自动释放失败时会保留本次完整结算后的捕获数据，避免丢失已推进的麻醉、生命体征或实体快照；已为 0 耐久的后续重试不会每秒无意义改写物品数据。如果暂时找不到安全位置、任一已加载维度中已存在同 UUID 实体或实体数据无法重建，会保留捕获数据并在后续结算中重试，不复制实体。

玩家可以对已放置且有恐龙的捕获笼右键使用麻醉针，复用麻醉系统的注射队列延长麻醉时间。使用主模组 diet 能接受的食物右键时，会通过 `JSMetabolismModule` / `Diet` 验证食物并喂食；使用水桶右键时，会在口渴系统可用且未满时补水，并把空桶返还给玩家或丢到附近。

恐龙博士眼镜看向已放置捕获笼时，会向服务端请求笼内恐龙的精简观察快照，并用同一套 HUD 显示年龄、血量、性别、饥饿、口渴、心情、麻醉状态、基因，以及额外的“笼体耐久”和“被捕获时长”。该网络快照只包含 HUD 所需字段，不发送完整实体 NBT。缺失或非法数据只隐藏对应字段，不影响捕获笼本身。
...
`snapshotTag` 是精简 HUD 快照，只包含显示名称、年龄推算、血量、性别、饥饿、口渴、心情、麻醉时间、基因列表和被捕获时长，不包含完整实体 NBT。客户端正缓存 20 tick 内直接使用；20 到 200 tick 内继续显示旧快照并后台刷新，超过 200 tick 或收到 unavailable 后隐藏捕获笼 HUD。缓存随断线、客户端世界卸载和 HUD 缓存清理一起释放。
...
- `assets/jsrevise/blockstates`、`models/block` 和 `textures/block`：捕获笼多方块整体外观资源；16 个 part 模型是空 BER 占位模型，真实 2×4×2 外观由客户端 controller 方块实体渲染器使用连续大面贴图绘制。
- `assets/jsrevise/models/item`：四个物品及麻醉弩三段拉弦、六档剩余弹量状态模型。
- `assets/jsrevise/textures/item`：16×16 物品纹理，包括眼镜正视图及麻醉弩待机、三段拉弦和六档剩余弹量状态；捕获笼物品栏显示由物品模型直接复用方块贴图。
...
- 新增恐龙捕获笼。它只捕获 active 麻醉中的 `JSAnimalBase`，使用 controller + part 多方块实现 2×4×2 逻辑单体和真实占地/碰撞，保存完整实体 NBT 与原 UUID，释放时恢复同一只实体；新放置和新捕获生成的正门朝向玩家所在侧，旧存档已放置笼子不自动旋转迁移。资源为偏白灰集装箱风格，六张主面贴图统一外框；正门保留双开门、中心门缝、横梁、左右窗和上方横向黄色锁扣，两张侧面贴图一致并带黄色圆章和深色恐龙剪影，物品与创造栏图标使用复用方块贴图的 baked 长方体模型。破坏任意 part 会清除整个结构；创造模式破坏有恐龙的已放置笼子会保留数据掉落，空手 Shift 右键已放置笼子不会释放恐龙，非玩家移除、爆炸或其他模组替换捕获笼时会兜底保留数据掉落。放置方块、背包、掉落物和打开容器会结算麻醉剩余时间、捕获时长和笼体耐久；笼体耐久最大 `1000`，active 麻醉期间不消耗，耐久归零会自动释放并消费载体，释放失败会保留本次 settled 数据以便重试，释放前的 UUID 冲突检查覆盖所有已加载维度。恐龙博士眼镜看向已放置捕获笼时，通过 `NETWORK_VERSION=6` 的服务端权威精简快照显示笼内恐龙 HUD，并额外显示笼体耐久和被捕获时长；资源测试覆盖贴图尺寸、主面外框、正门/侧面/物品模型和空 part 模型。
